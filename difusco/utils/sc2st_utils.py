...
#         # 在这里，我们使用scatter_add来聚合邻居节点的特征
#         return scatter(inputs, index, dim=0, reduce='mean')


if __name__ == '__main__':
  import sys
  sys.path.append('/home/liujing/code/STDD/difusco/co_datasets')
  from scCube_sc_st_graph_dataset import scCube_data
  from torch_geometric.loader import DataLoader as GraphDataLoader
  from tqdm import tqdm
  import pandas as pd

  path_file = '/media/imin/DATA/jjingliu/STDD/tensor_simulation_by_scCube/'
  test_inds = list(range(9000,9001))

  train_dataset = scCube_data(path_file, test_inds)
  ...
  for [i_batch, batch] in tqdm(enumerate(train_dataloader), total=len(train_dataloader)):
    ...
    idx, count_sc, count_st, assignment_matrix, cell_type_sc, cell_type_st, pos, i = batch
    
    st_ct_path = '/media/imin/DATA/jjingliu/STDD/csv_data/cell_type_st_post/cell_type_st_post9000.csv'
    sc_ct = pd.read_csv(st_ct_path,encoding="utf-8")
    sc_ct = torch.tensor(sc_ct.to_numpy()).to(dtype=torch.float32)

    sc_ct_predict = deconv(assignment_matrix, 'FLAGS', cell_type_sc)

    sc_ct_predict, sc_ct = sc_ct_predict[0].to('cpu'), sc_ct.to('cpu')
    RMSE_sc, RMSE_global = RMSE_deconv(sc_ct_predict, sc_ct)

    print(RMSE_sc, RMSE_global)
    print(sc_ct_predict.shape, sc_ct_predict)
    print(sc_ct.shape, sc_ct)
