...
class CategoricalDiffusion(object):
  ...
  def __init__(self, T, schedule):
    # Diffusion steps
    # self.QsQ_bar [T+1, 2, 2]; self.QsQs [T, 2, 2]; self.Qsbeta [T,]
  ...
  def sample(self, x0_onehot, t):
    # Select noise scales
    # t [bsz]; Q_bar[bsz, 2, 2]
    # x0_onehot [bsz, 1, num_nodes, num_nodes, 2]
    # xt [bsz, bsz, num_nodes, num_nodes, 2]
    # torch.bernoulli(xt[..., 1].clamp(0, 1)): [bsz, bsz, num_nodes, num_nodes]
